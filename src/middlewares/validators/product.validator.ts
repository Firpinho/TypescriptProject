...
export const validateProductPOST = (req:Request, res:Response, next:NextFunction): void => {
    const {error} = productSchemaPost.validate(req.body, {abortEarly: false});
    error ? res.status(404).send(error) : next();
}


export const validateProductGET = (req:Request, res:Response, next:NextFunction): void => {
